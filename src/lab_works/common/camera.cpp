...
namespace M3D_ISICG
{
	...
	{
		Vec2f flat_disp = Vec2f( _invDirection.x, _invDirection.z );
		flat_disp = glm::normalize( flat_disp );
		_position -= Vec3f(flat_disp.x, 0, flat_disp.y) * p_delta;
		_computeViewMatrix();
	}
	...
	{ 
		_viewMatrix = glm::lookAt( _position, - _invDirection + _position, _up );
	}
	...
	Vec3f Camera::getLookAt() {
		...
		return -_invDirection + _position;
